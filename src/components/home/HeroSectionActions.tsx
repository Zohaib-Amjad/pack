...
export default function HeroSectionActions({ primaryCta, secondaryCta }: HeroSectionActionsProps) {
  ...
  const primaryLabel = (primaryCta.label || "Get Your FREE Quote").replace(/\bfree\b/gi, "FREE");
  const secondaryLabel = (secondaryCta.label || "View All Products").replace(/\bsee all products\b/gi, "View All Products");
  ...
  return (
    <div className="flex flex-row items-center justify-start gap-3">
      {primaryIsQuote ? (
        ...
        >
          <ArrowRight size={14} strokeWidth={2.5} /> {primaryLabel}
        </button>
      ) : (
        <Link
