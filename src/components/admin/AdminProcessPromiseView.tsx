...
export default function AdminProcessPromiseView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsProcessSection("promise");
  const { toast } = useToast();

  const promise = local || DEFAULT_CMS_PROCESS.promise;

  const [sectionLabel, setSectionLabel] = useState(promise.sectionLabel);
  const [titleLead, setTitleLead] = useState(promise.titleLead);
  const [titleAccent, setTitleAccent] = useState(promise.titleAccent);
  const [description, setDescription] = useState(promise.description);
  const [cardsJson, setCardsJson] = useState(
    JSON.stringify(promise.cards || [], null, 2)
  );
  ...
  useEffect(() => {
    if (local) {
      ...
      setCardsJson(JSON.stringify(local.cards || [], null, 2));
    }
  }, [local]);
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      const fallback = DEFAULT_CMS_PROCESS.promise;
      setSectionLabel(fallback.sectionLabel);
      setTitleLead(fallback.titleLead);
      setTitleAccent(fallback.titleAccent);
      setDescription(fallback.description);
      setCardsJson(JSON.stringify(fallback.cards || [], null, 2));
    }
    toast({
      title: "Reset from server",
      description: "Restored latest Promise settings from Supabase.",
    });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Quality promise has been saved & published.",
      });
