...
export default function AdminAboutManufacturingView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsAboutSection("manufacturing");
  const { toast } = useToast();

  const manufacturing = local || DEFAULT_CMS_ABOUT.manufacturing;

  const [sectionLabel, setSectionLabel] = useState(manufacturing.sectionLabel);
  const [titleLead, setTitleLead] = useState(manufacturing.titleLead);
  const [titleAccent, setTitleAccent] = useState(manufacturing.titleAccent);
  const [body, setBody] = useState(manufacturing.body);
  const [highlightsText, setHighlightsText] = useState(
    (manufacturing.highlights || []).join("\n")
  );
  const [factoryImageUrl, setFactoryImageUrl] = useState(
    manufacturing.factoryImageUrl || ""
  );
  const [factoryImageAlt, setFactoryImageAlt] = useState(
    manufacturing.factoryImageAlt || ""
  );
  ...
  useEffect(() => {
    if (local) {
      ...
      setBody(local.body);
      setHighlightsText((local.highlights || []).join("\n"));
      setFactoryImageUrl(local.factoryImageUrl || "");
      setFactoryImageAlt(local.factoryImageAlt || "");
    }
  }, [local]);
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      const fallback = DEFAULT_CMS_ABOUT.manufacturing;
      ...
      setHighlightsText((fallback.highlights || []).join("\n"));
      setFactoryImageUrl(fallback.factoryImageUrl || "");
      setFactoryImageAlt(fallback.factoryImageAlt || "");
    }
    toast({
      title: "Reset from server",
      description: "Restored latest Manufacturing settings from Supabase.",
    });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: "Manufacturing copy has been saved & published.",
    });
                ...
                <div className="cb p-6">
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <input
                        className="w-full min-h-[40px] px-[14px] py-2 text-[13px] bg-[#f5f3ee]/50 border border-[#e0ddd6] rounded-[8px] focus:outline-none focus:border-[#2d5c3e] text-[#1a1a1a]"
                        value={factoryImageAlt}
                        onChange={(e) => setFactoryImageAlt(e.target.value)}
                      />
                    </div>
                  </div>
                </div>
