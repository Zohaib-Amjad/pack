...
export default function AdminProcessHeroView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsProcessSection("hero");
  const { toast } = useToast();

  const hero = local || DEFAULT_CMS_PROCESS.hero;
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      const fallback = DEFAULT_CMS_PROCESS.hero;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: "Process hero copy has been saved & published.",
    });
