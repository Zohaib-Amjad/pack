...
import type { CmsAboutCert } from "@/types/cms";

export default function AdminAboutCertificationsView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsAboutSection("certifications");
  const { toast } = useToast();

  const certifications = local || DEFAULT_CMS_ABOUT.certifications;

  const [sectionLabel, setSectionLabel] = useState(certifications.sectionLabel);
  const [titleLead, setTitleLead] = useState(certifications.titleLead);
  const [titleAccent, setTitleAccent] = useState(certifications.titleAccent);
  const [description, setDescription] = useState(certifications.description);
  const [items, setItems] = useState<CmsAboutCert[]>(
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      const fallback = DEFAULT_CMS_ABOUT.certifications;
    ...
    toast({
      ...
      description: "Restored latest Certifications settings from Supabase.",
    });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: "Certifications copy has been saved & published.",
    });
    setTimeout(() => setIsSaved(false), 3000);
  };

  const addItem = () => {
    const newItem: CmsAboutCert = {
      id: `cert-${Date.now()}`,
      title: "New Certification",
      desc: "Certification details and description.",
      rating: "",
      active: true,
    };
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="min-w-0">
            <form onSubmit={handleSave} className="flex flex-col gap-4 max-w-[920px]">
              ...
              {/* Card 2: Certification Items List */}
              <div className="card bg-white/80 backdrop-blur-md border border-[#e0ddd6]/80 rounded-[16px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
                <div className="ch p-[18px_24px] border-b border-[#e0ddd6]/60 flex items-center justify-between bg-[#f5f3ee]/30">
                  <div className="ch-l flex-1">
                    <div className="ct font-display text-[14px] font-bold text-[#1a1a1a] tracking-tight">
                    ...
                    <div className="cs text-[11px] text-[#aaa6a0] mt-[2px] font-medium uppercase tracking-wider">
                      Toggle Active to hide on the About page
                    </div>
                  </div>
                </div>
                ...
                <div className="cb p-0">
                  <div className="flex flex-col gap-0 border-b border-[#e0ddd6] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-[11px] font-semibold leading-relaxed text-[#7a7672]">
                      These map to the 3 certification cards on the About page.
                    </p>
                    <button
                      type="button"
                      onClick={addItem}
                  ...
                  <div className="divide-y divide-[#e0ddd6]">
                    {items.map((item, idx) => (
                      <div key={item.id || idx} className="space-y-3 px-4 py-4">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            {item.active === false && (
                              <span className="inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider bg-[#f0ede8] text-[#aaa6a0]">
                                Hidden
                              </span>
                            )}
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={item.rating || ""}
                              onChange={(e) =>
                                updateItem(idx, "rating", e.target.value)
                              }
                            />
                          </div>
                        </div>
