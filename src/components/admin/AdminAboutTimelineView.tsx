...
import type { CmsAboutTimelineItem } from "@/types/cms";

export default function AdminAboutTimelineView() {
  const { local, setLocal, save, saving, refetch } = useCmsAboutSection("timeline");
  const { toast } = useToast();

  const timeline = local || DEFAULT_CMS_ABOUT.timeline;

  const [sectionLabel, setSectionLabel] = useState(timeline.sectionLabel);
  const [titleLead, setTitleLead] = useState(timeline.titleLead);
  const [titleAccent, setTitleAccent] = useState(timeline.titleAccent);
  const [items, setItems] = useState<CmsAboutTimelineItem[]>(
    timeline.items || []
  );
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      const fallback = DEFAULT_CMS_ABOUT.timeline;
    ...
    toast({
      ...
      description: "Restored latest Timeline settings from Supabase.",
    });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: "Company timeline has been saved & published.",
    });
    setTimeout(() => setIsSaved(false), 3000);
  };

  const addItem = () => {
    const newItem: CmsAboutTimelineItem = {
      id: `ab-time-${Date.now()}`,
      year: new Date().getFullYear().toString(),
      title: "New Milestone",
      desc: "Describe what happened during this milestone.",
      active: true,
    };
  ...
  const updateItem = (
    index: number,
    field: keyof CmsAboutTimelineItem,
    value: unknown
  ) => {
    const updated = [...items];
              ...
              <div className="card bg-white/80 backdrop-blur-md border border-[#e0ddd6]/80 rounded-[16px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
                <div className="ch p-[18px_24px] border-b border-[#e0ddd6]/60 flex items-center justify-between bg-[#f5f3ee]/30">
                  <div className="ch-l flex-1">
                    ...
                    <div className="cs text-[11px] text-[#aaa6a0] mt-[2px] font-medium uppercase tracking-wider">
                      Toggle Active to hide timeline milestones
                    </div>
                  </div>
                </div>
                ...
                <div className="cb p-0">
                  <div className="flex flex-col gap-0 border-b border-[#e0ddd6] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-[11px] font-semibold leading-relaxed text-[#7a7672]">
                      Year, title, and description per row.
                    </p>
                    <button
                      type="button"
                      onClick={addItem}
