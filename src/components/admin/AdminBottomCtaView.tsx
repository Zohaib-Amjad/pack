...
export default function AdminBottomCtaView() {
  const [title, setTitle] = useState("Let's Make Something Great");
  const [subtitle, setSubtitle] = useState(
    "Tell us about your project. We'll handle design, production, and shipping, start to finish."
  );
  const [buttonLabel, setButtonLabel] = useState("Get Your Free Quote");
  const [isSaved, setIsSaved] = useState(false);

  const handleReset = () => {
    setTitle("Let's Make Something Great");
    setSubtitle(
      "Tell us about your project. We'll handle design, production, and shipping, start to finish."
    );
    setButtonLabel("Get Your Free Quote");
  };
                ...
                <div className="ch p-[18px_24px] border-b border-[#e0ddd6]/60 flex items-center justify-between bg-[#f5f3ee]/30">
                  <div className="ch-l flex-1">
                    ...
                    <div className="cs text-[11px] text-[#aaa6a0] mt-[2px] font-medium uppercase tracking-wider">
                      Band before footer on homepage
                    </div>
                  </div>
                </div>
