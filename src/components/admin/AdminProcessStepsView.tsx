...
export default function AdminProcessStepsView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsProcessSection("steps");
  const { toast } = useToast();

  const [jsonText, setJsonText] = useState(
    JSON.stringify(local || DEFAULT_CMS_PROCESS.steps, null, 2)
  );
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      setJsonText(JSON.stringify(DEFAULT_CMS_PROCESS.steps, null, 2));
    }
    toast({
      title: "Reset from server",
      description: "Restored latest Process steps from Supabase.",
    });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Process steps have been saved & published.",
      });
                ...
                <div className="ch p-[18px_24px] border-b border-[#e0ddd6]/60 flex items-center justify-between bg-[#f5f3ee]/30">
                  <div className="ch-l flex-1">
                    ...
                    <div className="cs text-[11px] text-[#aaa6a0] mt-[2px] font-medium uppercase tracking-wider">
                      JSON array · icons: file|palette|package|truck · details: string[]
                    </div>
                  </div>
                </div>
