...
export default function AdminPortfolioHeaderView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsPortfolioSection("header");
  const { toast } = useToast();

  const header = local || DEFAULT_CMS_PORTFOLIO.header;

  const [sectionLabel, setSectionLabel] = useState(header.sectionLabel);
  const [titleLead, setTitleLead] = useState(header.titleLead);
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      const fallback = DEFAULT_CMS_PORTFOLIO.header;
    ...
    toast({
      ...
      description: "Restored latest Portfolio header from Supabase.",
    });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: "Portfolio page header has been saved & published.",
    });
