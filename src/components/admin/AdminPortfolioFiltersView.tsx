...
import { useCmsPortfolioSection } from "@/hooks/useCmsPortfolioSection";
import { DEFAULT_CMS_PORTFOLIO } from "@/data/cms-defaults";
import { useToast } from "@/hooks/use-toast";

export default function AdminPortfolioFiltersView() {
  const { local, setLocal, save, saving, refetch } =
    useCmsPortfolioSection("filterLabels");
  const { toast } = useToast();

  const filterLabels = local || DEFAULT_CMS_PORTFOLIO.filterLabels;

  const [text, setText] = useState((filterLabels || []).join("\n"));
  ...
  const handleReset = () => {
    if (refetch) {
      ...
      setText((DEFAULT_CMS_PORTFOLIO.filterLabels || []).join("\n"));
    }
    toast({
      title: "Reset from server",
      description: "Restored latest filter labels from Supabase.",
    });
  };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    const parsedLabels = text
      .split("\n")
      .map((l) => l.trim())
      .filter((l) => l.length > 0);

    setLocal(parsedLabels);
    if (save) {
      await save(parsedLabels);
    }
    setIsSaved(true);
    toast({
      title: "Saved successfully",
      description: "Portfolio filter labels have been saved & published.",
    });
                ...
                <div className="ch p-[18px_24px] border-b border-[#e0ddd6]/60 flex items-center justify-between bg-[#f5f3ee]/30">
                  <div className="ch-l flex-1">
                    ...
                    <div className="cs text-[11px] text-[#aaa6a0] mt-[2px] font-medium uppercase tracking-wider">
                      One label per line — first is usually “All”
                    </div>
                  </div>
                </div>
