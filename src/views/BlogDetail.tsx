...
export default function BlogDetail({ slug }: { slug: string }) {
  const fallbackPost = DEFAULT_BLOG_POSTS[slug];
  ...
  const fallbackRelated = useMemo(() => {
    const allPosts = Object.values(DEFAULT_BLOG_POSTS).filter((p) => p.slug !== slug);
  ...
  const displayRelated = (related && related.length > 0) ? related : fallbackRelated;
    ...
    return (
      <Layout>
        <div className="container-max px-4 sm:px-6 py-40 text-center">
          <p className="font-display text-5xl font-bold text-foreground mb-4">Post Not Found</p>
          <p className="font-sans text-sm text-muted-foreground mb-10">This article may have been moved or unpublished.</p>
          <Link href="/blog" className="inline-flex items-center gap-2 font-sans text-sm font-semibold text-accent hover:underline">
    ...
    <Layout>
      ...
      <section className="bg-primary relative overflow-hidden">
        ...
        <div className="container-max px-4 sm:px-6 lg:px-8 pt-10 pb-16 sm:pt-14 sm:pb-20 relative z-10">
          <Link href="/blog" className="inline-flex items-center gap-1.5 font-sans text-xs text-white/40 hover:text-white/70 transition-colors mb-8 no-underline">
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
                  <p className="font-sans text-[9.5px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Article Info</p>
                  <div className="space-y-3">
                    ...
                    {post.category && (
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="font-sans text-[10px] text-muted-foreground">Category</p>
                ...
                <div className="bg-primary rounded-2xl p-5 text-center">
                  <p className="font-display text-lg font-bold text-white mb-1">Ready to order?</p>
          ...
          <div className="container-max px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-4 mb-10">
              <div>
                ...
                <h2 className="font-display text-3xl font-bold text-foreground">More in {post?.category || "Packaging"}</h2>
              </div>
              <div className="flex-1 h-px bg-border hidden sm:block" />
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {displayRelated.map((p: any) => (
                <Link key={p.id || p.slug} href={`/blog/${p.slug}`} className="group block no-underline">
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
                        <span className="font-sans text-[10.5px] text-muted-foreground flex items-center gap-1">
                          <Clock size={10} /> {readingTime(p.content || "")} min read
                        </span>
